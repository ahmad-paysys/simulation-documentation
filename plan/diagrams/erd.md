...
- **Run-number allocation:** `UNIQUE (suite_id, generation_id, run_number)` enforced by DB; computed under `pg_advisory_xact_lock(hashtext(suite_id::text || ':' || generation_id::text))`.
